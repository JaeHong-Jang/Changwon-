...
- 필요한 코드만 작성하고, 여러 곳에서 반복하는 계산·처리는 함수로 묶는다.
- 한 번만 쓰는 단순 전달·변환은 호출 위치에 직접 쓴다.
- 주석은 대상이나 역할을 짧게 적는다. 예: `# Shapefile을 이루는 파일 확장자`, `# 원본 파일 집합 하나의 형태`.
- 인코딩·고정 목록처럼 코드에서 드러나는 선택은 길게 설명하지 않는다.
- 산식·단위·검증 자료 분리 등 해석에 필요한 조건은 남긴다.
