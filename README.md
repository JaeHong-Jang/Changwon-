...
집중호우·태풍 발생 시 **침수 취약지역 + 하수도 역류 위험 + 취약계층 분포**를 100m 격자로 통합 분석하여
창원시가 우선적으로 점검·대응해야 하는 **TOP 20 지역**을 산출하는 **데이터 분석 프로젝트**

> 이 대회는 데이터 분석 공모전(제출물=분석보고서). AI는 분석 안에서 필요한 자리에만 사용 —
> ① 민원 원문 LLM 구조화(Layer 2 입력) ② XGBoost/SHAP 역류 예측. 알림 Agent는 활용방안의 출력부 예시(≤1p, 옵션).
