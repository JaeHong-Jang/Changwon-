...
밖의 농경지였다. Layer 1 은 도시 내수침수 지수이므로 그 표본에서는 점수가 낮을 수밖에 없었고,
...
| 연도 | 사상 | 양성 격자 | AUC | 상위 20% 포착률 | 불투수면 | 예상 침수심 | 펌프장 거리 |
|---|---|---:|---:|---:|---:|---:|---:|
...
(불투수면·예상 침수심·펌프장 거리는 그 사상에서 잠긴 격자들의 **중앙값**이다.)

**네 사상은 기준(0.70)을 넘고 두 사상은 무작위(0.5)보다 낮다.** 그 두 사상만 공통점이
있다 — 잠긴 곳이 **불투수면 0%, 시 예상 침수심 0, 펌프장 14km 밖**이다. 침수 원인도
2006년은 19건 전부 "집중호우로 인한 농경지 침수", 2025년은 20건 중 18건이
"내수_방재시설 용량부족"으로, 둘 다 **농경지 배수**다.

2014년이 흥미롭다. 불투수면 0%, 펌프장 10km 밖인데도 AUC 0.786 이다. 원인이
"외수범람·하천범람"이고 잠긴 격자의 하천 거리 중앙값이 **100m**(최솟값)였다.
...
재해취약성분석을 따랐고(§2), 창원시 침수예상도 역시 시가지를 대상으로 만들어졌다.
농경지 침수는 배수개선사업의 영역이지 도시 우수관리의 영역이 아니다.
...
2. **사상별 분해** (§7.3.6). 사상 하나씩 따로 채점해 성능이 특정 호우에 기대고 있는지
   확인했다. 결과: 6개 중 4개가 기준 통과, 2개(농경지 침수)가 미달.
...
| 13 | 농경지 침수는 잡지 못한다 | 2개 사상이 무작위 이하 | 도시 지수임을 명시, 농경지는 배수개선 영역 |
