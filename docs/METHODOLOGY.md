...
**9월 16일 침수흔적도를 확보해 이 상황은 끝났다(§7.3).** 2020~2023년 실제 침수 지구와 날짜가 생기므로
...
4. **양성만 있고 음성이 없다.** 신고·조사되지 않은 침수는 기록되지 않는다.
   AUC 는 상한이 아니라 하한에 가깝다.
...
1. **검증 침수를 늘린다.** 행안부 API 는 2018 년까지이고 창원 5개 연도뿐, 시 회신은 2025 년
   1건, 민원은 비공개다. 마감(9/30) 전에는 경로가 없다.
...
| 11 | 흔적도에 음성 라벨 없음 | 미신고 침수는 음성으로 셈 | AUC 를 하한으로 해석 |
