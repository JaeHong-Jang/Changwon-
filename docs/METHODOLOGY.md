...
#### 7.3.4 격자 642칸은 표본 642개가 아니다

침수흔적은 폴리곤 하나가 격자 수십 칸을 덮는다. 붙어 있는 격자는 **같은 침수 한 건**이므로
독립 관측이 아니다. 그런데 ROC-AUC 와 일반적인 신뢰구간은 관측이 독립이라고 가정한다.
이것을 무시하면 유사복제(pseudo-replication)가 되어 불확실성을 과소평가한다.
...
| **독립 덩어리** | **46개** |
| 덩어리 크기 중앙값 | 3칸 |
| 가장 큰 덩어리 | 86칸 (전체의 13.4%) |

**유효 표본은 642가 아니라 46이다.** 재표본은 격자가 아니라 덩어리 단위로 해야 한다
(cluster bootstrap). 두 방식의 차이는 이렇다.

| 재표본 단위 | AUC 95% 신뢰구간 | 폭 |
|---|---|---:|
| 격자 (틀린 방식) | [0.733, 0.768] | 0.035 |
| **덩어리 (맞는 방식)** | **[0.637, 0.863]** | **0.226** |

**6.5배 차이다.** 격자 단위 구간을 보고했다면 "게이트를 여유 있게 통과했다"고 쓰게 되는데
사실이 아니다. 덩어리 단위로 보면 **게이트 기준 0.70 이 신뢰구간 안에 들어 있다.**

> 점추정 0.750 은 기준을 넘었다. 그러나 **"0.70 을 확실히 넘었다"고는 말할 수 없다.**
> 두 문장을 모두 보고서에 쓴다.

구현은 `src/data/uncertainty.py` 다.

#### 7.3.5 순환 — 성능의 상당 부분이 시 침수예상도에서 온다

창원시 침수예상도는 Layer 1 의 **입력**이다(`flood_l210_100_depth_m`). 그래서
"우리 지수가 실제 침수를 잘 맞혔다"는 문장에는 **시 모형의 성과가 섞여 있다.**

흔적 폴리곤이 예상도 구역과 얼마나 겹치는지 재 보면 사상별 AUC 가 거의 그대로 설명된다.
...
이것을 분리하려면 예상도 단독 점수와 나란히 놓아야 한다. 같은 라벨로 세 가지를 채점했다.

| 지수 | AUC | 상위 20% 포착률 |
|---|---:|---:|
| **A. 시 침수예상도 단독** | **0.709** | 0.520 |
| B. Layer 1 현행 (예상도 포함) | **0.750** | 0.544 |
| C. Layer 1 − 예상도 | 0.734 | 0.505 |
| D. Layer 1 − 예상도·하천·복개 | 0.696 | 0.458 |

읽는 법은 셋이다.

1. **시가 이미 가진 예상도만으로 AUC 0.709 가 나온다.** 우리 지수의 증분은 **+0.041** 이다.
2. 예상도를 빼도 0.734 가 남는다(C). 지형·피복 변수가 독립적으로 기여한다는 뜻이며,
   우리가 예상도를 베끼기만 한 것은 아니다.
3. **그러나 46개 덩어리 기준 신뢰구간이 겹친다** — 예상도 단독 [0.589, 0.826],
   Layer 1 [0.637, 0.863]. 이 표본으로는 **둘을 통계적으로 구분할 수 없다.**

$$\text{증분 AUC} = 0.750 - 0.709 = +0.041 \quad (\text{95\% 신뢰구간 겹침})$$

**보고서에 쓸 문장은 이것이다.** "AUC 0.750 을 달성했다"가 아니라
**"시가 이미 가진 침수예상도가 0.709 이고, 여기에 지형·인구·취약성을 더해 0.750 이 됐다.
표본이 46개 사상이라 이 차이는 통계적으로 확정되지 않는다."**

이 표를 빼면 남의 성과를 우리 것으로 보고하게 된다. 코드가 매 실행마다 이 세 값을
함께 계산한다 (`h06_layers._incremental_value`).

**그래도 우리 산출물이 의미를 갖는 이유**는 성능 숫자가 아니라 **산출물의 성격**에 있다.
예상도는 "어디가 잠기는가"만 말하고, CDRI 는 거기에 **누가 사는가(E)·얼마나 취약한가(V)·
대응역량이 있는가(D)** 를 결합해 **우선순위**를 만든다. 예상도로는 TOP 20 을 뽑을 수 없다.
성능 경쟁이 아니라 **의사결정 단위를 바꾸는 것**이 기여다 — 그렇게 쓴다.
...
1. **유효 표본이 46개 사상이다.** 격자 642칸은 표본 642개가 아니다(§7.3.4).
   AUC 95% 신뢰구간이 [0.637, 0.863] 로 넓고 게이트 기준 0.70 을 포함한다.
2. **성능의 상당 부분이 입력에서 온다.** 시 예상도 단독 0.709 대비 증분은 +0.041 이고
   신뢰구간이 겹친다(§7.3.5).
...
### 7.5 등급 경계 캘리브레이션 — 구현은 됐고, 검증할 표본이 부족하다 (2026-09-16)

#### 7.5.1 무엇을 하려던 것인가

Jenks 자연구분은 **점수 분포**의 단절만 본다. "R5 와 R4 가 실제로 얼마나 다르게
잠기는가"는 답하지 못한다. 결정 003 이 본안으로 고른 3안은 그 질문에 답한다 —
등급 경계를 점수가 아니라 **관측된 침수 발생률**로 맞춘다.

#### 7.5.2 순환을 어떻게 막았나

흔적으로 경계를 맞추고 **같은** 흔적으로 채점하면 in-sample 이라 성능이 부풀려진다.
그래서 사상을 시간으로 나눴다. 분할 규칙은 결과를 보기 전에 정했다 — 연도를 시간순으로
세워 앞 절반이 경계 산출, 뒤 절반이 채점. 홀수면 여분은 경계 산출 쪽에 준다.
창원 기록은 연도마다 사상이 정확히 하나씩이라 연도 분할이 곧 사상 분할이다.

| | 사상 | 양성 격자 |
|---|---|---:|
| 캘리브레이션 (경계 산출) | 2006 에위니아 · 2012 산바 · 2014 국지성호우 | 440 |
| 검증 (채점) | 2016 차바 · 2019 미탁 · 2025 7.19 호우 | 251 |
| 양쪽 모두 | — | 49 |

#### 7.5.3 경계를 정하는 절차

**① 등온회귀(PAV)로 단조 발생률 곡선을 적합한다.** 원자료의 발생률은 표본 잡음으로
오르내리는데, 그대로 두면 잡음을 경계로 삼게 된다. "위험이 높을수록 더 자주 잠긴다"는
단조성은 지수의 전제이므로, 그 전제를 만족하는 곡선 중 자료에 가장 가까운 것을 쓴다.

$$\hat{p} = \arg\min_{q_1 \le q_2 \le \cdots \le q_n} \sum_{i=1}^{n} (y_i - q_i)^2$$

여기서 $y_i$ 는 CDRI 오름차순 $i$ 번째 격자의 침수 여부다. 제약하의 최소제곱해이며
pool-adjacent-violators 로 $O(n)$ 에 구한다 (Ayer et al. 1955).

**② 목표 백분위 근처의 단절점으로 경계를 옮긴다.** 목표는 98/90/70/40 백분위
(= 설계 비율 2/8/20/30/40%)이고, 그 지점에서 $\pm 3\%p$ 안에 PAV 곡선의 단차가 있으면
단차가 가장 큰 곳으로 옮긴다. 없으면 목표 백분위를 그대로 쓴다.

**③ 인접 등급 발생률 비가 1.3 미만이면 통합을 권고한다.** 경계를 억지로 만들지 않는다.

$\pm 3\%p$ 와 1.3 은 설계 파라미터라 `config/config.yaml` 에 두고,
$\pm 2/3/5\%p \times 1.2/1.3/1.5$ 아홉 조합으로 민감도를 함께 보고한다
(`src/data/calibration.py`).

#### 7.5.4 결과 — 검증 사상 채점 (점추정)

R5 경계만 PAV 단차로 옮겨졌고(98 → 99.31 백분위), 나머지 셋은 목표 백분위에
단차가 없어 그대로 남았다.

| 등급 | 격자 | 검증 양성 | 발생률 | lift | 인접 비 |
|---|---:|---:|---:|---:|---:|
...
기저 발생률 1.76%. **점추정만 보면** 단조가 성립하고 Cochran-Armitage $z = 5.45$,
R5/R1 발생률 비 11.2배다. **그러나 이 숫자들은 §7.5.5 에서 무효가 된다.**

경계의 안정성: $\pm 2\%p$ 와 $\pm 3\%p$ 에서 등급 크기가 **완전히 동일**했고,
$\pm 5\%p$ 에서만 R3/R4 경계가 움직였다(2041/950 → 2527/464). R5 경계는 아홉 조합
전부에서 70칸으로 고정이다.

#### 7.5.5 점추정의 함정 — R5 의 양성 9칸은 침수 **1건**이다

위 표만 보면 "단조성 성립, $p = 4.9 \times 10^{-8}$, R5 는 기저의 7.3배"라고 쓰고 싶어진다.
**처음에 그렇게 썼고, 틀렸다.**

Cochran-Armitage 검정은 격자가 독립이라고 가정한다. 그런데 §7.3.4 에서 봤듯 붙어 있는
격자는 같은 침수 한 건이다. 검증 기간 양성을 덩어리로 묶으면 이렇다.

| | 격자 | 덩어리(독립 침수) |
|---|---:|---:|
| 검증 양성 전체 (순위대상) | 180 | **12** |
| 그중 R1 | 47 | 10 |
| 그중 R2 | 53 | 8 |
| 그중 R3 | 47 | 9 |
| 그중 R4 | 24 | 4 |
...
(한 침수가 여러 등급에 걸치면 각 등급에 하나로 센다. 합이 12 보다 크다.)

**"R5 lift 7.29" 는 침수 한 건이 R5 격자 9칸에 걸친 결과다.** 그 한 건이 다른 곳에서
일어났다면 R5 lift 는 0 이다. 덩어리를 복원추출해 다시 재면 이렇게 된다.

| 지표 | 점추정 | 덩어리 부트스트랩 95% 구간 |
|---|---:|---|
| R5 lift | 7.29 | **[0.00, 22.62]** |
| 5등급이 모두 단조로 나오는 재표본 비율 | — | **35.9%** |

**구간 하한이 0 이고, 단조성은 재표본의 1/3 에서만 유지된다.** 이 표본으로는
"등급이 오를수록 더 잠긴다"도, "R5 는 확실히 더 위험하다"도 말할 수 없다.
$p = 4.9 \times 10^{-8}$ 은 침수 12건을 180건으로 센 결과라 **무효**다.

#### 7.5.6 그러면 무엇을 말할 수 있나

| 주장 | 판정 | 근거 |
|---|:--:|---|
| 등급 경계를 발생률로 맞추는 절차를 구현했다 | 가능 | 코드·테스트·민감도 9조합 |
| 경계가 설계 파라미터에 안정적이다 | 가능 | ±2·±3%p 에서 등급 크기 완전 동일 |
| 등급이 오를수록 실제로 더 잠긴다 | **불가** | 단조 재표본 35.9% < 95% |
| R5 는 기저보다 확실히 더 잠긴다 | **불가** | lift 구간 하한 0 |
| R3 과 R4 는 구분된다 | **불가** | 점추정 비 1.10, 구간은 더 넓다 |
| 자료가 3단 구조를 뒷받침한다 | **불가** | 3단도 같은 표본에 기댄다 |

앞서 "자료가 뒷받침하는 구조는 3단"이라고 썼는데 그것도 점추정에 기댄 서술이었다.
**철회한다.** 12개 침수 덩어리로는 5단이든 3단이든 등급 간 차이를 검증할 수 없다.

따라서 등급 체계의 위상은 이렇게 정리된다.

> **R1~R5 는 실증적으로 검증된 위험 구분이 아니라, 발생률 캘리브레이션 절차로 정한
> 재현 가능한 행정 우선순위 구간이다.** 경계를 정하는 규칙과 그 안정성은 제시할 수 있지만,
> 등급 간 실제 위험 차이는 이 표본(검증 기간 침수 12건)으로 확정할 수 없다.

코드가 이 판단을 강제한다. `reporting_constraint` 의 `monotone_claim_allowed` 와
`top_grade_separable` 이 둘 다 거짓이면 해당 주장을 보고서에 쓰지 않는다
(`h07_cdri._reporting_constraint`, 기준 `MONOTONE_SHARE_MIN = 0.95`).

#### 7.5.7 왜 본안을 Jenks 로 되돌리지 않았나

같은 검증 자료로 Jenks 도 채점했다. 점추정으로는 Jenks 가 단조성부터 깨진다
(R4 1.70% < R3 2.12%). 하지만 **이 비교 역시 같은 12건에 기대므로 어느 쪽이 낫다고
판정할 근거가 되지 못한다.** 검증 결과로 본안을 고르지 않는다는 원칙은 그대로다 —
그렇게 하면 검증 자료가 선택에 개입한다. 본안은 결정 003 이 자료를 보기 전에 정한
캘리브레이션이다.

방식 간 일치도(2차 가중 $\kappa$): 캘리브레이션–Jenks **0.802**, 캘리브레이션–Balica 0.474.
두 방식이 등급을 근본적으로 다르게 주지는 않는다. 갈리는 곳은 R4/R5 경계다.

#### 7.5.8 이 한계를 어떻게 줄일 수 있나

검증 침수가 12건이라는 것이 근본 원인이다. 늘리는 길은 셋이다.

1. **행안부 API 의 나머지 기간.** DSSP-IF-00117 은 2002~2018 전국 자료인데 창원에서
   5개 연도만 나왔다. 2020~2024 창원 사상은 이 API 에 없고, 시 회신분도 2025 년 1건뿐이다.
2. **민원 이의신청**(§9 한계 9). 민원은 침수보다 훨씬 촘촘하게 남는다. 다만 비공개 결정을
   뒤집어야 한다.
3. **공간 해상도를 낮춘 검증.** 500m 블록이나 행정동 단위로 올리면 덩어리가 합쳐져
   유효 표본은 오히려 줄어든다. 해결책이 아니다.

현실적으로 9월 30일 마감 전에 검증 표본을 늘릴 방법은 없다. 따라서 보고서는
**등급 체계를 '검증된 위험 구분'이 아니라 '재현 가능한 우선순위 구간'으로 제시한다.**
...
1. **등급 경계의 시간 분할 검증** (§7.5). 앞 사상(2006·2012·2014)으로 경계를 정하고
   ...
   out-of-sample 이다. 결과: 점추정은 단조였으나 검증 침수가 12건(R5 는 1건)이라
   덩어리 단위로는 단조성이 재표본의 35.9% 에서만 유지됐다. **검증되지 않았다.**
...
| # | 한계 | 영향 | 완화 |
|---|---|---|---|
| 1 | 유효 표본이 46개 덩어리 | AUC 95% 구간 [0.637, 0.863] 이 기준 0.70 을 포함 | 덩어리 부트스트랩으로 구간 보고 (§7.3.4) |
...
| 8 | 등급 간 위험 차이를 검증할 표본 부족 (검증 침수 12건, R5 는 1건) | 단조성·R5 분리 주장 불가 | 등급을 '재현 가능한 우선순위 구간'으로 제시 (§7.5.6) |
...
| 14 | 성능의 상당 부분이 시 예상도에서 온다 | 증분 AUC +0.041, 구간 겹침 | 예상도 단독 점수 병기, 기여를 성능이 아니라 우선순위 산출로 규정 (§7.3.5) |
