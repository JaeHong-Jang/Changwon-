...
**본안 선택은 조건부다.** 원래 설계는 실제 침수 발생률로 경계를 맞추는 캘리브레이션이지만,
그러려면 침수흔적 양성 격자가 100개 이상 있어야 한다. 지금은 **0개**이므로 자동으로
Jenks 자연구분이 본안이 되고, Balica 고정 경계와 고정 백분위는 비교용으로 병기한다.
이 분기는 코드가 판정한다 (`src/data/grades.py: choose_scheme`).
...
| # | 한계 | 영향 | 완화 |
|---|---|---|---|
| 1 | 외적 검증 완료 (AUC 0.750, 포착률 54.4%) | 성능 주장 가능 | §7.3, 아래 10·11·12 한계 병기 |
...
| 10 | 흔적 시점(2006~2019) ≠ 지수 시점(2025) | 검증이 retrodiction | §8.2 사후검증으로 완화 |
