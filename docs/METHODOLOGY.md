...
### 7.3 침수흔적 검증 — 자료는 왔으나 판정할 수 없었다 (2026-09-16)

정보공개청구로 받은 침수흔적도가 도착했다. 형식은 벡터(SHP)라 읽을 수 있었지만
**내용이 우리가 검증하려던 것과 달랐다.**

| 기대 | 실제 수령분 |
|---|---|
| 2020~2023 지구별 69건 | **2025년 7월 19일 집중호우 단일 사상** |
| 도시·농촌 전역 | 의창구 **대산면 17곳 + 동읍 1곳** |
| 여러 사상 → 시간 분할 가능 | 사상 1개 → 시간 분할 불가 |

폴리곤 18개, 총 0.069 km². 침수 원인은 전부 **내수(방재시설 용량부족)** 다.
격자 면적의 10% 넘게 겹치는 칸이 25개이고, 그중 **순위 대상(인구·주택 있는 칸)은 2개**뿐이다.

**이 표본으로는 성능을 판정하지 않는다.** 양성 격자가 판정 기준(30칸) 미달이므로
코드가 `label_sufficient = false` 를 기록하고 게이트를 걸지 않는다
(하네스 H06 의 "라벨 부족 시 예측 성능 주장 금지로 전환" 분기).

참고값으로 계산한 ROC-AUC 는 **0.365** 로 무작위(0.5)보다 낮다. 왜 그런지 진단했다.

| 변수 | 침수 25칸 중앙값 | 창원 전체 중앙값 | 해석 |
|---|---:|---:|---|
| 표고 | 6.2 m | 83.2 m | 지형 변수는 저지대로 **정확히 잡았다** |
| 경사 | 0.19° | 11.4° | 평지로 정확히 잡았다 |
| TWI | 11.5 | 7.2 | 물이 모이는 곳으로 정확히 잡았다 |
| 불투수면 | 0% | 0% | 농경지 |
| 시 침수예상 침수심 | 0 | 0 | **시 모형도 0/25 를 잡지 못했다** |
| 펌프장 거리 | 14.7 km | 5.6 km | 서비스권 밖 |

지형 세 변수는 이 지점들을 정확히 저지대·평지·물 모임으로 잡았다. 그런데 도시 변수
세 개(불투수면·예상 침수심·펌프장 서비스권)가 모두 0이라 합산에서 밀렸다.
**창원시가 수리모형으로 만든 침수예상도조차 이 25칸을 하나도 잡지 못했다.**
도시침수정보시스템이 시가지를 대상으로 하기 때문이다.

**결론.** 우리 Layer 1 은 도시 내수침수 지수이고, 이 흔적은 농촌 배수시설 용량부족이다.
서로 다른 현상이므로 이 자료는 우리 지수를 반증하지도, 입증하지도 못한다.
다만 **AUC 0.365 는 숨기지 않고 표본 조건과 함께 보고한다.** 그리고 이것은
지수의 **적용 범위**를 보여주는 근거가 된다 — 이 지수는 시가지용이다.

**도시 침수 사상 자료가 여전히 필요하다.** 남은 경로는 행안부 재난안전데이터공유플랫폼
침수흔적도 API(DSSP-IF-00117, 2002~2018 전국 38,003건)이며 회원가입과 활용신청이 필요하다.
엔드포인트는 `https://www.safetydata.go.kr/V2/api/DSSP-IF-00117?serviceKey=...` 로 확인했다.
