...
게이트는 자료를 보기 **전에** 하네스에 못박아 둔 값이다(AUC ≥ 0.70, 상위 20% 포착률 ≥ 50%).
...
$$\text{AUC} = P\bigl(L_1(\text{침수 격자}) > L_1(\text{비침수 격자})\bigr) = 0.750$$

$$\text{lift} = \frac{P(\text{침수} \mid \text{상위 20\%})}{P(\text{침수})}
= \frac{0.0231}{0.0085} = 2.72$$

읽는 법: 상위 20% 격자 15,080칸이 실제 침수 642칸 중 **349칸(54.4%)**을 담았다.
무작위로 20%를 고르면 20%만 담긴다. 즉 **2.72배** 효율이다.

순위대상만 보면 AUC 0.713, 포착률 44.2%로 떨어진다. 인구·주택이 있는 칸끼리는
서로 비슷해 변별이 어렵기 때문이다. **두 값을 모두 보고한다.** 전체 기준만 쓰면
"산지를 제외해 얻은 점수"라는 지적을 받을 수 있고, 순위대상 기준만 쓰면 지수가
저지대를 골라내는 능력을 과소평가한다.

#### 7.3.4 9월 16일의 AUC 0.365 는 무엇이었나

같은 코드로 시 회신분만 넣었을 때 AUC 는 0.365 — 무작위(0.5)보다 낮았다.
숫자가 0.365 → 0.750 으로 바뀐 이유는 **지수를 고쳐서가 아니라 표본이 바뀌어서**다.
지수 쪽 코드와 가중치는 그대로다.

| 변수 | 침수 격자 중앙값 | 창원 전체 중앙값 | 9/16 표본(농촌 25칸) |
|---|---:|---:|---:|
| 표고 | 6.3 m | 83.2 m | 6.2 m |
| 경사 | 0.50° | 11.35° | 0.19° |
| TWI | 10.93 | 7.16 | 11.5 |
| 불투수면 비율 | **0.45** | 0.00 | **0.00** |
| 시 침수예상 침수심 면적비 | **0.08** | 0.00 | **0.00** |
| 펌프장 거리 | 3.6 km | 5.6 km | **14.7 km** |

지형 세 변수(표고·경사·TWI)는 두 표본에서 모두 저지대·평지·물 모임을 정확히 잡았다.
갈린 것은 **도시 변수 세 개**다. 9/16 표본은 불투수면 0%, 예상 침수심 0, 펌프장 14.7 km
밖이었다(불투수면이 0 보다 큰 칸도 12% 뿐). Layer 1 은 도시 내수침수 지수이므로 그 표본에서는 점수가 낮을 수밖에 없었고,
실제로 **창원시 자신의 수리모형 침수예상도조차 그 25칸을 하나도 잡지 못했다.**

이번 표본은 불투수면 중앙값 45%, 예상 침수심 면적비 0.08, 펌프장 3.6 km 로 시가지다.
시 침수예상도(L210 100년빈도)는 642칸 중 **334칸(52%)**을 덮었다. 참고로만 적는다 —
예상도는 면적 제약이 없고 우리 지표는 상위 20%로 제한한 값이라 직접 비교가 아니다.
요점은 9/16 표본에서 0/25 였던 예상도가 이번 표본에서는 절반을 덮었다는 것, 즉
**두 표본이 서로 다른 종류의 침수**라는 진단이 맞았다는 데 있다.

**따라서 9/16 결과는 반증이 아니라 적용 범위의 경계였다.** 이 지수는 시가지용이다.
보고서에는 0.365 와 0.750 을 **둘 다** 싣고, 무엇이 달랐는지 위 표로 설명한다.

#### 7.3.5 이 검증의 한계 — 세 가지

1. **시점이 어긋난다.** 흔적은 2006~2019년(+2025년 1건)이고, 지수의 지형·피복·펌프장은
   2025년 현재 상태다. 그 사이 개발과 하수도 정비가 있었다. 이것은 예측이 아니라
   **현재 지수로 과거 사건을 되짚은 것(retrodiction)**이며, 2019년 이전 피복으로
   지수를 다시 만든 사후검증(§8.2)이 이 한계를 줄인다.
2. **양성만 있고 음성이 없다.** 흔적도에 없는 칸이 "안 잠겼다"는 보장은 없다. 신고·조사가
   안 된 침수는 기록되지 않는다. 그래서 AUC 는 **상한이 아니라 하한**에 가깝다.
3. **사상별 강우가 다르다.** 12개 사상의 강우 규모가 제각각인데 하나로 합쳤다.
   사상별 분리 평가는 사상당 양성 격자가 적어(중앙값 수십 칸) 아직 못 한다.

이 세 가지를 §9 한계표에 그대로 옮겼다.

#### 7.3.6 사상별로 나눠 보면 성능이 갈린다 — 이것이 지수의 적용 범위다

합산 AUC 0.750 은 12개 사상을 한 덩어리로 채점한 값이다. 큰 사상 하나가 좋으면
나머지가 나빠도 높게 나오므로, **사상별로 따로 채점했다.** 음성은 "어느 사상에서도
잠기지 않은 격자"로 뒀다. 다른 사상에서 잠긴 칸을 음성으로 세면 맞힌 것을 틀렸다고
채점하게 되기 때문이다.

| 연도 | 사상 | 양성 격자 | AUC | 상위 20% 포착률 |
|---|---|---:|---:|---:|
| 2006 | 에위니아 | 107 | **0.389** | **0.000** |
| 2012 | 산바 | 206 | 0.792 | 0.558 |
| 2014 | 국지성 집중호우 | 132 | 0.786 | 0.508 |
| 2016 | 차바 | 170 | 0.920 | 0.935 |
| 2019 | 미탁 | 56 | **0.962** | 0.982 |
| 2025 | 7.19 집중호우 | 25 | **0.367** | **0.000** |

**포착률 0.000 은 반올림이 아니라 진짜 0 이다.** 2006 년 침수 격자 107칸의 L1 최댓값이
0.3683, 2025 년 25칸의 최댓값이 0.3480 인데 상위 20% 경계가 0.4188 이다. **가장 높은
칸조차 경계에 못 미쳐** 단 한 칸도 상위 20% 에 들지 못했다. 두 사상의 격자는 한 구역에
붙어 있어 점수가 거의 같다 (L1 표준편차 0.026·0.031, 창원 전체는 0.124).

#### 무엇이 갈랐나 — 중앙값이 아니라 비율로 봐야 한다

처음에는 잠긴 격자의 **중앙값**으로 진단했는데 그 방식이 틀렸다. 불투수면 같은 변수는
0 에 몰린 양봉분포라, 절반을 조금 넘는 칸이 0 이면 중앙값이 0 이 되어 "전부 농경지"처럼
보인다. 실제로는 절반이 시가지일 수 있다. 조건을 만족하는 **칸의 비율**로 다시 냈다.
...
**불투수면은 결정적 변수가 아니다.** 2006 년 침수 격자의 불투수면 > 0 비율이 27.1% 로
창원 전체(28.4%)와 사실상 같다. "농경지라서 못 잡았다"는 설명은 맞지 않는다.

**갈린 것은 두 변수다.** 실패한 두 사상만 **시 침수예상도 커버리지 0.000 이고 펌프장
1km 내 비율도 0.000** 이다. 나머지 넷은 예상도 0.27~1.00, 펌프 0.076~0.98 이다.
정확히 말하면 이렇다 — **창원시가 수리모형으로 침수를 예상한 적이 없고 배수펌프장
서비스권에도 들지 않는 곳에서 잠겼을 때, 우리 지수는 그곳을 짚지 못한다.**

2014 년은 도시 변수가 약한데도(예상도 0.273, 펌프 0.076) AUC 0.786 이다.
**하천 200m 내 비율이 0.758 로 창원 전체 0.207 의 3.7배**다. 원인 기록도
외수범람·하천범람이다. 도시 변수가 약해도 하천 근접 변수가 끌어올린 경우다.

정리하면 이 지수가 잡는 것과 못 잡는 것은 이렇다.

| 잡는다 | 못 잡는다 |
|---|---|
| 시 침수예상도가 덮은 구역 | 예상도 밖에서 일어난 침수 |
| 배수펌프장 서비스권 안 | 서비스권 밖 (1km 초과) |
| 하천 200m 이내 | — |

**이것은 실패가 아니라 경계다.** Layer 1 은 설계 단계부터 국토부 지침의 **도시** 기후변화
재해취약성분석을 따랐고, 입력인 창원시 침수예상도 역시 시가지를 대상으로 만들어졌다.
입력이 덮지 않은 영역을 산출이 덮을 수는 없다. 다만 **"농촌이라 못 잡는다"가 아니라
"시가 예상하지 못한 곳은 우리도 예상하지 못한다"** 가 정확한 서술이며, 후자가 훨씬
불편한 문장이므로 그대로 쓴다.

**보고서에는 합산값과 이 표를 반드시 함께 싣는다.** 합산 AUC 0.750 만 쓰면 두 사상이
무작위 이하라는 사실을 숨기는 것이 된다. 코드가 이것을 강제한다 — `events_below_min`
이 비어 있지 않으면 Layer 1 이 안내문에 미달 사상 목록을 박아 넣는다
(`src/stages/h06_layers.py: _event_spread`).

**사후에 범위를 좁혀 점수를 올리지 않았다.** "시가지만 골라 다시 계산하면 AUC 가
오른다"는 계산은 하지 않는다. 검증 자료를 본 뒤 대상을 고르는 것이기 때문이다.
적용 범위는 서술로 남기고 숫자는 있는 그대로 싣는다.
...
### 7.5 등급 경계 캘리브레이션 — 단조성은 확인, 등급 분리는 R5 만 (2026-09-16)
...
기저 발생률 1.76%. 단조성 성립, Cochran-Armitage $z = 5.45$, $p = 4.9 \times 10^{-8}$,
R5/R1 발생률 비 **11.2배**.
...
#### 7.5.5 통과하지 못한 기준 — 숨기지 않는다

사전에 정한 다섯 기준 중 **셋은 통과, 둘은 미달**이다.

| 기준 | 값 | 판정 |
|---|---|:--:|
| 등급별 발생률 단조 증가 | 성립 | 통과 |
| 추세 유의 ($p < 0.01$) | $4.9\times10^{-8}$ | 통과 |
| R5 lift $\ge 3$ | 7.29 | 통과 |
| 인접 발생률 비 모두 $\ge 1.3$ | R3→R4 가 **1.10** | 미달 |
| R4 lift $\ge 2$ | **1.43** | 미달 |

기준을 사후에 낮추지 않는다(재튜닝 금지). 대신 **주장 범위를 줄인다.** 코드가
`reporting_constraint` 로 이것을 산출해 보고서 문장을 제한한다.

- 말할 수 있는 것: **등급이 오를수록 실제로 더 자주 잠긴다**(검증 사상에서 확인).
- 말할 수 있는 것: **R5 는 기저의 7.3배, R1 의 11.2배로 잠긴다.**
- 말할 수 **없는** 것: R3 과 R4 는 서로 다르다. 발생률이 갈리지 않는다.

따라서 자료가 뒷받침하는 실무 구조는 5단이 아니라 **3단**이다.

| 통합 단계 | 구성 | 격자 | 검증 발생률 | lift |
|---|---|---:|---:|---:|
| 1 관찰·일반관리 | R1+R2+R3 | 9,182 | 1.60% | 0.91 |
| 2 우선대응 | R4 | 950 | 2.53% | 1.43 |
| 3 최우선대응 | R5 | **70** | **12.86%** | **7.29** |

5등급 표기는 하수관로 상태등급(1~5)과 방향을 맞추기 위해 유지하되, **성능을 주장할 때는
3단 구조로 말한다.**

#### 7.5.6 왜 검증에 진 방식으로 갈아타지 않았나

같은 검증 자료로 Jenks 도 채점해 봤다.

| | 단조성 | R5 lift | R5/R1 | 미달 기준 |
|---|:--:|---:|---:|---:|
| 캘리브레이션 (본안) | 성립 | 7.29 | 11.2 | 2개 |
| Jenks (2안) | **깨짐** (R4 1.70% < R3 2.12%) | 2.73 | 5.07 | 4개 |

캘리브레이션이 네 기준 모두에서 낫다. 그렇다고 **검증 결과를 보고 본안을 고르지는
않는다.** 그러면 검증 자료가 선택에 개입해 더는 out-of-sample 이 아니게 된다.
본안은 결정 003 이 자료를 보기 전에 정한 것이고, 검증은 "무엇을 주장해도 되는지"만
정한다. Jenks 수치는 비교 정보로만 싣는다.

방식 간 일치도(2차 가중 $\kappa$): 캘리브레이션–Jenks **0.802**, 캘리브레이션–Balica 0.474.
Landis & Koch(1977) 기준 캘리브레이션과 Jenks 는 'almost perfect' 구간이라, 두 방식이
등급을 근본적으로 다르게 주는 것은 아니다. 갈리는 곳은 R4/R5 경계다.
...
1. **등급 경계의 시간 분할 검증** (§7.5). 앞 사상(2006·2012·2014)으로 경계를 정하고
   ...
   out-of-sample 이다. 결과: 단조 성립, $z = 5.45$, R5 lift 7.29.
...
| # | 한계 | 영향 | 완화 |
|---|---|---|---|
| 1 | 외적 검증의 표본이 12개 사상뿐 | AUC 0.750 의 신뢰구간이 넓다 | §7.3, 아래 10·11·12 병기 |
...
| 8 | R3·R4 발생률 미분리 (인접 비 1.10) | 5등급 개별 주장 불가 | 3단 통합 구조로 보고 (§7.5.5) |
...
| 12 | 사상별 성능 편차 큼 (AUC 0.37~0.96) | 합산값만 쓰면 과대 진술 | 사상별 표 병기, 적용 범위 명시 (§7.3.6) |
| 13 | 시 예상도·펌프권 밖 침수는 잡지 못한다 | 2개 사상이 무작위 이하 | 입력 범위를 넘을 수 없음을 명시 (§7.3.6) |
