...
코드 커밋 상태 재실행 (M6, `docs/q1/M6.md`): run_id `holdout_20260926T164805Z_7f7968f_84ef30a7`.
게이트 해시가 위 실행과 같아 아래 표의 수치는 모두 같다. 다만 사전 고정 규칙(`docs/q1/M6_protocol.md` §2.3)으로는
**불일치**다. 실행 메타 `union_area_km2` 가 2.0e-13 km² 다르고, 면적·객체 단위 지표 1,966행이 최대 1.8e-12 다르다
(흔적 재투영 좌표의 마지막 자리 차이로 추정). 그래서 인용 실행은 위 9/24 실행으로 둔다.
...
| 자료 | 전체 | 도심 | 농경지 | 면적 중앙값 |
|---|---:|---:|---:|---:|
| 개발 145개 (`label_audit_development_20260924T104040Z_1b8106ba`) | 95.2% | — | — | 8,750 m² |
| 홀드아웃 196개 (`label_audit_holdout_20260924T104628Z_1b8106ba`) | 66.3% | 12.3% | 97.5% | 3,590 m² |

(run_id 정정 2026-09-27, M6: 이전 판은 저장되지 않은 `…095654Z_40523c56`·`…095656Z_40523c56` 을 인용했다.
표의 값은 저장된 `artifacts/evaluation/label_audit/*_summary.json` 과 같다.)
...
| 6 | HEAD 전체 재실행 재현성 | 완료 — H10 노드가 빈 산출물 재실행·산출물 비교·홀드아웃 불변·환류 기록을 자동 검사 (run 20260924T163108Z-e25f3f2 통과) |
| 7 | C2 원인 분해: 폴리곤별 면적·토지이용·교차 격자·규칙별 포함 여부 추적표 | 완료 (`src/models/label_audit.py`) |
| 8 | 연대표: 지수 동결 시점, 입력 자료 시점, 흔적 입수 시점, 최초 채점 | 클라우드 초안 완료 (`docs/q1/M6_chronology.md`, M6). 로컬 `.omc/` 기록으로 보강 필요 |

### P1 — Q1 수준으로 올리는 것

- ~~**다도시 재현 (C5)**~~ **2026-09-27 생략 결정.**
  - M1 에서 C-b 가 폐기되고 M4 분해식이 나온 뒤, 창원 단일 사례 연구로 쓴다.
  - 절차 초안: `docs/q1/M5_protocol.md`. 미등록·미실행. 논문에는 재현을 주장하지 않고 한계로 적는다.
  - 원래 계획:: 행안부 API 전국 흔적에서 도시 선정 기준(자료 완전성·도시구조·침수유형)을 성능 확인 전에 고정하고, 창원에서 얻은 평가 규약을 그대로 적용한다. 창원 지수 없이도 "지형 기준 점수 + 단위별 평가"로 C2 를 시험할 수 있다.
