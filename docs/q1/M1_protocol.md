...
**보정 (2026-09-26, 첫 커밋 `b2b9cdc` 뒤, 어떤 M1 AUC 도 계산하기 전).** 라벨 없는 진단에서 문제를 찾았다.
`fill_sinks` 는 채운 싱크를 완전 평탄하게 남긴다. 그래서 D8 규칙에서 내리막 이웃이 없는 칸이 23,501개 생겼다.
전 격자망 칸의 74% 가 하천에 닿기 전에 그 평탄 칸에서 흐름이 끊겼다. 그대로 두면 HAND 는 "배수로 위 높이"가 아니라
"가장 가까운 웅덩이 위 높이"가 된다. 그래서 다음과 같이 고친다.
- 흐름 방향만 Priority-Flood+ε (Barnes et al. 2014, `fill_sinks` 와 같은 논문의 변형; 채운 칸에 다음 부동소수 증분)
  DEM 에서 같은 D8 규칙으로 구한다. 높이(HAND 값)는 ε 없는 `fill_sinks` 표고로 잰다 (두 표고의 최대 차 1.1e-13 m).
- 민감도 하천망의 유량누적도 같은 ε 방향으로 센다.
- 보정 뒤 흐름 끝 칸은 2,525개(대부분 격자망 가장자리)다. 분석 격자 75,400칸 중 80.8% 가 OSM 하천에 닿고,
  19.2% 는 해안·가장자리에서 흐름이 끝난다.
- 확인한 것: 재계산 표고 = `elev_m` (최대차 0.0). ε 없는 방향 규칙의 유량누적 = `flow_acc_cells` (최대차 0.0).
