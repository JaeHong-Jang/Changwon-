"""행정안전부 재난안전데이터공유플랫폼 침수흔적도 수집 (DSSP-IF-00117).

전국 38,003건을 페이지로 받아 창원 5개 구만 남긴다. **API 에 지역 필터가 없어서**
(`STDG_SGG_CD` 등을 넘겨도 전건이 돌아온다) 전건을 훑고 로컬에서 거른다.

응답의 `GEOM` 은 EPSG:3857(웹 메르카토르) WKT 다. 분석 좌표계 EPSG:5179 로 바꿔 저장한다.
인증키는 `.env` 의 `SAFETYDATA_API_KEY` 에서 읽으며 코드·로그에 남기지 않는다.
"""
...
# 침수 원인이 내수(배수 불량)인지 구분하는 토큰. Layer 1 은 내수를 겨냥한다.
