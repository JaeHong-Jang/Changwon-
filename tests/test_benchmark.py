...
from src.models.folds import (
    ...
    loeo_groups, past_flood_score, spatial_folds,
)
...
class FoldTest(unittest.TestCase):
    def test_loeo_groups_ignore_held_event_labels(self):
        points, labels, _ = synthetic()
        points = np.vstack([points, [[1990, 1900], [2090, 1900]]])
        training = np.r_[labels, False, False]
        events = np.column_stack([np.zeros(len(points), dtype=bool), training])
        changed = events.copy()
        changed[-2:, 0] = True
        groups, background = loeo_groups(points, events, 0)
        other_groups, other_background = loeo_groups(points, changed, 0)
        np.testing.assert_array_equal(groups, other_groups)
        np.testing.assert_array_equal(background, other_background)
        # 공통 학습 행에서 내부 CV도 동일해야 한다.
        train = np.flatnonzero(~changed[:, 0])
        before = spatial_folds(points[train], training[train], groups=groups[train], n_splits=3)
        after = spatial_folds(points[train], training[train], groups=other_groups[train], n_splits=3)
    ...
    def test_proximity_uses_training_positives_only(self):
        ...
        points = np.array([[50., 50.], [150., 50.], [25., 25.]])
        traces = gpd.GeoDataFrame({"event_year": ["2006", "2012"], "role": ["development"] * 2},
                                 geometry=[box(0, 0, 100, 100), box(140, 40, 160, 60)], crs=5179)
        scores = past_flood_score(points, traces, np.array([1, 2]), ["2006"])
        np.testing.assert_allclose(scores, [-50, 0])
        traces.loc[1, "geometry"] = box(-1000, -1000, 1000, 1000)
        np.testing.assert_allclose(past_flood_score(points, traces, np.array([1, 2]), ["2006"]), scores)

    def test_spatial_proximity_excludes_test_polygons(self):
        import geopandas as gpd
        from shapely.geometry import box

        points = np.array([[50., 50.], [450., 50.], [850., 50.]])
        traces = gpd.GeoDataFrame({"event_year": ["2006"] * 3, "role": ["development"] * 3},
                                 geometry=[box(0, 0, 100, 100), box(400, 0, 500, 100),
                                           box(0, 20, 900, 80)], crs=5179)
        scores = past_flood_score(points, traces, np.array([1, 2]), ["2006"], train=np.array([0]))
        np.testing.assert_allclose(scores, [-350, -750])

    def test_empty_proximity_source_is_constant(self):
        import geopandas as gpd

        points = np.array([[0., 0.], [100., 0.]])
        traces = gpd.GeoDataFrame({"event_year": [], "role": []}, geometry=[], crs=5179)
        np.testing.assert_array_equal(past_flood_score(points, traces, np.array([1]), ["2006"]), [0])
